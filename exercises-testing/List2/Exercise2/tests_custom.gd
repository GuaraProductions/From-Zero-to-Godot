...
func _build_test_cases() -> Array[Dictionary]:
	return [
		_make_case("Times table structure for n=3", "validar_hierarquia_n3"),
		_make_case("Times table texts for n=3", "validar_textos_n3"),
		_make_case("Division is grouped correctly for n=1", "validar_divisao_n1")
	]
...
func validar_hierarquia_n3(resultado, instancia) -> Dictionary:
	...
	if not erros.is_empty():
		return _falha("The container hierarchy does not match the expected times table.", "VSeparator/VBoxContainer/VSeparator triplets for each group", "\n".join(erros))

	return _sucesso("Scene tree hierarchy is organized correctly for n=3")
...
func validar_textos_n3(resultado, instancia) -> Dictionary:
	...
	if not erros.is_empty():
		return _falha("The generated times table texts do not match the expected output.", "Correct expressions from 1 to 10 for n=3", "\n".join(erros))

	return _sucesso("Times table texts are correct for n=3")
...
func validar_divisao_n1(resultado, instancia) -> Dictionary:
	...
	if not erros.is_empty():
		return _falha("Division layout for n=1 is incorrect.", "10 division groups with 1 label each", "\n".join(erros))

	return _sucesso("Division is grouped correctly for n=1")
