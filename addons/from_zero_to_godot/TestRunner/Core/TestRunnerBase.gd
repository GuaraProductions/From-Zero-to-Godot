...
var test_config: TestConfigResource = null
...
func _carregar_dados_teste(arquivo_testes: String) -> bool:
	test_config = TestConfigFactory.load_from_json(arquivo_testes)
	if not test_config:
		return false

	dados_teste = test_config.to_dictionary()
	return true
...
func _comparar_exato(obtido, esperado) -> bool:
	if typeof(esperado) != TYPE_FLOAT and typeof(esperado) != TYPE_INT \
	 and typeof(obtido) != typeof(esperado):
		return false
