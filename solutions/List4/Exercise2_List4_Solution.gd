...
class Inventory:
	...
	func add_item(name: String,
		...
		# Create new item
		var new_item = Item.new(new_id, quantity, name, description, texture)
	...
	func remove_item(id: int, quantity: int) -> bool:
		...
		# If no items left, remove from dictionary
		if item.quantity <= 0:
			_items.erase(id)
	...
	func get_item_name_with_quantity(id: int) -> String:
		if _items.has(id):
			var item: Item = _items[id]
			return "%s (%d)" % [item.name, item.quantity]
		return ""

	func get_item_description(id: int) -> String:
		if _items.has(id):
			return _items[id].description
		return ""
